/* The native manifest vocabulary is declared once, in the module this
 * boundary publishes. Imported for the helpers below and re-exported
 * further down, so the IR reads as one vocabulary and nothing inside this
 * package has to know where it lives. */
import type {
  IrBytesElem,
  IrBytesType,
  IrNativeAbiType,
...
/** The same set as `IrNativeIntegerScalar`, as a value, for the places that
 * iterate it. The published module states the union and this is annotated
 * against it, so adding a name in one place and not the other fails to
 * compile rather than silently admitting a scalar nothing lowers. */
export const IR_NATIVE_INTEGER_SCALARS: readonly IrNativeIntegerScalar[] = [
  "i8",
  "u8",
  "i16",
  "u16",
...
export function isSupportedArrayElem(t: IrType): boolean {
  switch (t.kind) {
    ...
    case "classval":
      return true;
    /* This fork's two element kinds, both admitted by a program that needs
     * them rather than by symmetry. A native handle is a managed, ref-counted
     * cell exactly as a class instance is, and `tests/native-ir/handle-stored`
     * stores one in an array and reads the array's length while the cell stays
     * retained. `tests/corpus/bigint` declares a `bigint[]` literal.
     *
     * The other three kinds this fork adds — nativeScalar, nativeStruct and
     * nativePointer — stay refused, because nothing has needed them yet and a
     * precise refusal is the correct answer until something does. */
    case "nativeHandle":
    case "bigint":
      return true;
    default:
