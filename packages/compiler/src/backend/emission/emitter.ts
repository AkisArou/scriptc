...
import { cFnPtrCast, cNativeScalarLiteral, cType, releaseCallC, cStringLiteral, cDecl, vAdapters } from "./emit-types.js";
...
export class CEmitter {
  ...
  nullableUnionOf(typeId: string): NativeCallbackAbsentPayload {
    ...
    if (found === null) {
      throw new Error(`emitter bug: no "T | null" union for payload ${typeId}`);
    }
  ...
  emit(): string {
    ...
    /* Trampolines are emitted into a buffer for the same reason the bodies
     * are: emitting one can register an interned instance the file's header
     * has to declare. A withheld payload's null arm is such an instance —
     * nothing in the program names it, so it is first reached here, after the
     * unit table would already have been written. */
    const nativeCallbackDefs: string[] = [];
    this.emitNativeCallbackDefs(nativeCallbackDefs);
    ...
    if (this.mod.nativeTarget !== undefined) {
      out.push(
        `_Static_assert(sizeof(uintptr_t) * CHAR_BIT == ${this.mod.nativeTarget.pointerBits}, ` +
          `"Native IR target pointer width does not match the C backend target");`,
        ``,
      );
    }
  ...
  emitNativeCallbackDefs(out: string[]): void {
    for (const adapter of this.nativeCallbackAdapters.values()) {
      ...
      if (form.shape === "direct") {
        ...
        const args = payloads.map((payload) => {
          ...
          if (payload.absent === null) {
            handleCells.push(...build);
            return cell;
          }
          /* A payload the emitter may withhold. The handler is handed the
           * union rather than the object, so absence is a value it tests
           * instead of a pointer it must not read — and the null arm costs no
           * allocation, because every wrap of a unit arm is THE interned
           * immortal instance for that (union, tag).
           *
           * The cell is built only on the present arm. Interning a NULL
           * pointer would hand back whatever cell a previous NULL prepared,
           * which is the one way this could silently produce an object where
           * the library said there was none. */
          const arm: IrType = { kind: "nativeHandle", typeId: payload.typeId };
          const rc = vAdapters(arm);
          const value = `sc_arg${payload.slot}`;
          handleCells.push(
            `  ScrUnion *${value};`,
            `  if (sc_a${payload.slot} == NULL) {`,
            `    ${value} = ${this.unitInstanceRef(payload.absent.unionId, payload.absent.nullTag)};`,
            `  } else {`,
            ...build.map((line) => `  ${line}`),
            `    ${value} = scr_union_new_ref(${payload.absent.presentTag}, ${cell}, ` +
              `&${rc.retain}, &${rc.release}, ${this.traceArgC(arm)});`,
            `  }`,
          );
          return value;
        });
      ...
      if (form.shape === "queued") {
        ...
        const call = `(${cFnPtrCast(nativeCallbackSourceSignature(sourceType, (typeId) => this.nullableUnionOf(typeId).unionId))}sc_cb->fn)(sc_cb${args.length > 0 ? `, ${args.join(", ")}` : ""})`;
        out.push(
          ...handleCells,
          `  ${call};`,
