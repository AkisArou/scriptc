...
bool scr_loop_run(ScrPromise *top_level) {
  ...
  for (;;) {
    ...
    /* Two independent foreign liveness sources: a platform loop this runtime
     * is attached to, and the FFI queue's own pending posts. Either one
    ...
    if (scr_reffed_timers == 0 && scr_reffed_immediates == 0 && !scr_children_reffed_pending() && !io && !events && !net && !dgram && !watch && !attached && !ffi && !renames) break;
