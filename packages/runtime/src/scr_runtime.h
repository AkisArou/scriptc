...
/* Native IR error boundary (scr_lib.c). The errno snapshot call must be the
 * first call after an errno-sentinel result; formatting and exception
 * allocation happen only after the exact thread-local value is captured. */
