...
The archive (libmbedtls.a) is built lazily on the first TLS-using compile into `.cache/mbedtls-<version>-<flavor>-<target>/` next to this file — one flavor per lane (plain, asan) and native platform/architecture or explicit cross target, the libqjs.a pattern. TLS-free binaries never compile or link any of this.
