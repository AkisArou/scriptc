...
// array the checked-dynamic value (the DOM has real arrays; a dyn-element
...
// keyed-DOM paths — TypeScript sources exactly like the JS residue that
// always lowered this way.

// Literal construction into the annotated slot (the DOM array literal).
...
// The dispatched methods on the DOM array.
u.push("pushed");
...
// Records with unknown[] fields carry the DOM array in a dyn slot.
interface Bag {
  items: unknown[];
  label: string;
}
