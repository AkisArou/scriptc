...
// (SCRIPTC_SAN=1: ASan + the runtime RC audit): DOM trees built and dropped in
...
for (let i = 0; i < 200; i = i + 1) {
  // Parse + validate + extract; the DOM (including the "extra" subtree the
  ...
  // extracted, others just dropped (the whole DOM must free either way).
  ...
  // DOM (object → array → objects) must release when the loop iteration's
  // bindings die.
  ...
  // Parse failures unwind through the DOM mid-build; catch recovers.
  try {
