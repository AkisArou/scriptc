...
// scalars AND other dyn values (whole-DOM ===), and logical operators
// (ToBoolean over the DOM kind), and `throw` (the DOM node rides the
...
// copy — differential corpus), and FUNCTIONS box as the DOM's callable
...
// DOM representation (Maps, classes, bytes).
...
// `u as string | undefined` compiles now — the DOM holds a first-class
